...
export const apiConfig: { [key: string]: string } = generateApiConfig(apiVersions);

function generateApiConfig(apiVersions: ApiVersion[]): { [key: string]: string } {
  const config: { [key: string]: string } = {};

  apiVersions.forEach((version) => {
    Object.keys(version.endpoints).forEach((endpointName) => {
      const fullPath = `${version.version}${version.endpoints[endpointName].path}`;
      config[endpointName] = generateUrl(fullPath);
    });
  });
