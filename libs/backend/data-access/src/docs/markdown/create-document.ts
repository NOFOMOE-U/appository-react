...
import options from '../../middleware/permissions/shield/mock-options'
...
import { CollaborationOptions, DraggableSection, StickyNote } from '../tools/co-document-interfaces'
import {
    addDatabaseDiagrams,
    addFlowCharts,
    addOutlines,
    addParagraphs,
    addPlanningCharts,
} from '../tools/colab-document-options'
...
export async function createDocument(
  ...
  // Add content to the PDF, hande other configurations
    ...
    let defaultDocumentType: string = 'blank';
...
export function addStickyNotes(
  ...
  if (notes) {
      for (const note of notes) {
          if (note.position && note.font) { // Check if note.position is defined
