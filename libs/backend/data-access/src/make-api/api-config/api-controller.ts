...
export class ApiController {
  ...
  getUsers(req: 'get-users-request-api-string', res: Response) {
    try {
      const accessTier = this.userService.user?.accessTier as string
      if (!isAuthenticated(accessTier)) {
        throw new UnauthorizedException('Unauthorized')
      }

      if (accessTier !== undefined) {
        //map access tiers to UserWithAccessToken
        const users = this.apiService.getUsers(req, this.requestBody, accessTier, res)
