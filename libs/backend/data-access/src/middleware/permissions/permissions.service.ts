...
export class PermissionsService {
  ...
  isAuthorized(userRole: string, resourceType: string, action: string) {
    const userRolePermissions = permissionsMatrix[userRole];
    if (userRolePermissions && userRolePermissions[resourceType]) {
      return permissionsMatrix[userRole]?.[resourceType] ? [action] || false : false
    }
    return false
  }
